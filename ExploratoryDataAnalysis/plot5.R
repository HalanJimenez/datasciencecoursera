...
ggplot(vehicleBaltimoreEmissions, aes(x = year, y = totalEmissions)) +
        ggtitle("Total Emissions in Baltimore from motor vehicle sources \n from 1999 to 2008") +
        xlab("Year") +
        ylab("Total Emissions (thousands of tons)") +
        geom_point(aes(col = totalEmissions)) +
        geom_line(aes(col = totalEmissions)) +
        geom_text(aes(label = totalEmissions, y = totalEmissions), size = 3) +
